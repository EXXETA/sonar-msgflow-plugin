 ...
 * @author Arjav Shah
 */
public class MQHeaderNodeSensor extends AbstractSensor implements Sensor {
	...
	public void execute(SensorContext context) {
		 ...
		 for (InputFile inputFile : fs.inputFiles(fs.predicates().matchesPathPatterns(MessageFlowPlugin.FLOW_PATH_PATTERNS))) {
			...
			if(msgFlow.getMqHeaderNodes().size()>0) {
				createNewIssue(context, inputFile, "MQHeaderNodeDetection",
						"The 'MQ Header' node was detected. Usage of MQ Header node is discouraged, manipulate the headers using ESQL.");
			}
		}
	}
}
